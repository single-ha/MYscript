...
class SanjieTask(Task):
    ...
    CALIBRATION = {
        ...
        "templates": [
            ...
            ("qq_option", "答题选项按钮(任选其一)", "答题界面里任意一个选项按钮，脚本就点它作答本题。"
                                              "不标则必须在运行参数 loop.answer_pos 里配一个固定点位"),
            ("qq_done", "答题完成标志", "答完后出现的「今日已答完/次数已用完/结算」这类字样，识别到即停；不标时靠空闲兜底"),
            ("qq_close", "「关闭」按钮(可选)", "答题结束后的关闭/退出按钮；没有可不标"),
        ],
        "watchlist": False,
    }
    ...
    def _do_find_card(self, ctx, rec, loop, regions, threshold):
        ...
        def probe(scene, rect):
            ...
            if join is not None:
                ...
                ctx.log(f"找到卡片（{score:.3f}）→ 点「参加」（{join[2]:.3f}），直接进答题界面。", level="hit")
                return scan.ACCEPT, join
            ctx.log("认出卡片但没找到右侧「参加」（检查 qq_join 模板/阈值）。", level="warn")
    ...
    def _find_join_on_row(self, ctx, list_region, entry_screen_xy, threshold, loop):
        """在卡片所在【那张卡片】的右侧条带里匹配「参加」按钮(qq_join)。命中返回 (x,y,score)，否则 None。
        按行+只取条目右侧、且限制在条目所属卡片列内，抗滚动、抗「一排多张卡片」时扫进右邻卡片（默认两张一排）。"""
        join_tpl = self.flags.get("qq_join")
        entry_tpl = self.flags.get("qq_entry")
        if join_tpl is None:
            ctx.log("找「参加」失败：qq_join 模板未标定。", level="warn")
            return None
