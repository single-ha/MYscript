...
_VIRTUAL_SPECS = {
    ...
    "shared": ("公共区域（全局共享）", {
        "regions": [
            ...
            # 拓印临摹资产（tuoying_area / tuoying_title / tuoying_upload）已迁到「工具」页「拓印」（tasks.tuoying），
        ...
        "templates": [
            ...
            ("clock", "小闹钟(寻路)",
             ...
             "原来分别在「刷副本」「抓鬼」各自标定里各标一份，其实是同一个图标，现统一移到这里，标一次全通用。"),
            ...
            ("popup_close", "弹窗关闭按钮(×)", "(可选)活动/公告类弹窗右上角的「×」关闭按钮。脚本「弹窗守卫」在任务等待期会自动把它点掉、防止弹窗挡画面导致流程中断（点不掉会按 Esc 兜底）；不标=不启用弹窗守卫。所有任务共用。", True),
        ],
        "watchlist": False,
    }),
}
...
def grab_roi_on_app(app, cfg, prompt, with_crop=False, toast=None, alpha_windows=None):
    ...
    if wr is None:
        ...
        return None, None
    # 注意：这里【不得】顺带改写 targets.base_size——基准尺寸只由「设为基准」按钮显式设置，
    # 标定框选时静默覆盖会让用户发现基准「自己变了」（已踩坑）。
