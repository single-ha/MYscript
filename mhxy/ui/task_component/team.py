...
class Team(TaskComponentBase):
    ...
    def init(self):
        ...
        ctk.CTkLabel(txt_t, text="组队（共享）", font=T.fonts["h2"], text_color=T.TEXT).pack(anchor="w")
        team_tc = cfg_mod.task_config(data.cfg, "teaming")
        ...
        ctk.CTkButton(btns_t, text="标定（组队）", font=T.fonts["body"], height=36, width=120,
        ...
        self.btn_team = ctk.CTkButton(act, text="▶  一键组队", font=T.fonts["btn"], height=40, width=150,
                                      corner_radius=T.RADIUS_SM, fg_color=T.ACCENT, hover_color=T.ACCENT_HOVER,
                                      text_color=T.ON_ACCENT, command=self._start_teaming)
        self.btn_team.pack(side="left")
        # 一键解散：让所选各号都退出当前队伍（开队伍面板→退出队伍→关面板，每号同一套流程）
        self.btn_disband = ctk.CTkButton(act, text="⏏  一键解散", font=T.fonts["btn"], height=40, width=130,
        ...
        ctk.CTkButton(act, text="选择窗口/队长", font=T.fonts["body"], height=36, width=120,
                      corner_radius=T.RADIUS_SM, fg_color=T.BTN, hover_color=T.BTN_HOVER, text_color=T.TEXT,
                      border_width=1, border_color=T.BORDER,
                      command=lambda: util.open_window_picker(self.refresh, captain_ns="teaming")).pack(
            side="left", padx=(8, 0))
        # 队长ID 入口：带缩略图的小按钮，点开「队长ID 库」（当前+最近3历史可切换）；
        # 和刷副本页写同一处 teaming.leader_id，天然同步。
        self.btn_leader = ctk.CTkButton(act, text="标定队长ID", font=T.fonts["small"], height=36, width=110,
                                        corner_radius=T.RADIUS_SM, fg_color=T.BTN, hover_color=T.BTN_HOVER,
                                        text_color=T.TEXT, border_width=1, border_color=T.BORDER,
        ...
        self.lbl_team_status = ctk.CTkLabel(c_team, text="", font=T.fonts["small"],
    ...
    def _open_leader_gallery(self):
        util.open_leader_gallery(self._refresh_leader_btn)

    def _open_team_calibrate(self):
        """打开组队标定（共享命名空间 teaming）。队长ID 已移到「标定队长ID」按钮单独标，这里不再列出。"""
        util.calib_singleton("_team_cal_dialog", None, "打开组队标定失败", exclude=["leader_id"])
    ...
    def _render_team_action(self):
        ...
        if not (0 <= cap < n):
            cap = 0
        multi = data.cfg.get("targets", {}).get("multi", False)
        if n >= 2:
    ...
    def _start_teaming(self):
        ...
        if task_cls is None:
            ...
            return
        self.runner = TaskRunner(task_cls(), data.cfg)
