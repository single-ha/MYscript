...
class LeaderIdGallery(ctk.CTkToplevel):
    ...
    # ------------------------------------------------------------------
    def _render(self):
        if self.empty_bind is not None:
                T.unbind(self.empty_bind[0], self.empty_bind[1])
        from .util import load_thumb     # 延迟导入避免与 app.py 循环依赖
        ...
        if not history:
            ...
            self.empty_bind = T.bind_wraplength(empty)
            return
