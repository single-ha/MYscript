...
class SanjiePage(ctk.CTkFrame):
    ...
    def _build_body(self):
        ...
        hint = ctk.CTkLabel(left, text="点「参加」后直接进答题界面：脚本对每道题点任意一个选项，点完自动跳下一题（答错无所谓）。\n"
                               "识别到「完成」标志（今日已答完/次数用完等字样）即自动停；\n"
                               "找不到选项按钮超过一段时间也按答完处理。不标定选项模板时可在 loop.answer_pos 填固定点位。\n"
                               "鼠标甩到屏幕角落可紧急停止。",
    ...
    def refresh(self):
        ...
        need_t = ["qq_entry", "qq_join", "qq_option"]
        rdone = sum(1 for k in need_r if regions.get(k))
        tdone = sum(1 for k in need_t if templates.get(k))
        opt_ok = bool(templates.get("qq_option")) or bool(loopc.get("answer_pos"))
        self.lbl_calib.configure(
            text=f"标定：必要区域 {rdone}/{len(need_r)}，必要模板 {tdone}/{len(need_t)}"
                 + ("" if opt_ok else "，答题选项模板/固定点位缺")
                 + ("　✓ 可运行" if rdone == len(need_r) and tdone == len(need_t) and opt_ok else "　（还需标定）")
