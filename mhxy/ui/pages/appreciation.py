# -*- coding: utf-8 -*-
"""趣味鉴赏「配置/标定」卡：参数改动即保存，标定/重新加载即点即做。
...
class AppreciationConfig(ctk.CTkFrame):
    ...
    def __init__(self, master, app):
        ...
        ctk.CTkLabel(txt, text="趣味鉴赏", font=self.fonts["h2"], text_color=T.TEXT).pack(anchor="w")
        ctk.CTkLabel(txt, text="开活动→参加→匹配并点击心形图案，点满即停", font=self.fonts["small"],
        ...
        self.var_clicks, _ = param_entry(card, self.fonts, "点击次数上限", "30", self._on_target_clicks,
                                         tooltip="累计点满多少个心形图案就停")
    ...
    def refresh(self):
        ...
        loopc = tc.get("loop", {})
        self.var_clicks.set(str(loopc.get("target_clicks", 30)))
        ...
        _ready, txt, color = calib_status(regions=regions, templates=templates, task_name=self.TASK_NAME,
                                          region_keys=need_r, tpl_keys=need_t, tpl_opt_keys=opt_tpl, label="鉴赏标定")
        self.lbl_calib.configure(text=txt, text_color=color)

    # ---- 参数保存（改动即保存）----
    def _on_target_clicks(self, raw):
        self._save_loop(raw, "target_clicks", lambda v: max(1, int(float(v))), "30", self.var_clicks)
