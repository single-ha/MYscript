# -*- coding: utf-8 -*-
"""运镖「配置/标定」卡：参数改动即保存，标定/重新加载即点即做。
...
class EscortConfig(ctk.CTkFrame):
    TASK_NAME = "escort"
    LOG_SOURCE = "运镖"
    ...
    def __init__(self, master, app):
        ...
        ctk.CTkLabel(txt, text="运镖", font=self.fonts["h2"], text_color=T.TEXT).pack(anchor="w")
        ctk.CTkLabel(txt, text="开活动→参加→押送普通镖银，循环押满次数", font=self.fonts["small"],
        ...
        self.var_escorts, _ = param_entry(card, self.fonts, "押送次数（跑满即停）", "3", self._on_max_escorts,
                                          tooltip="循环押送普通镖银，累计成功多少次就停")
        self.var_time, _ = param_entry(card, self.fonts, "时间上限（分钟）", "60", self._on_time_limit,
    ...
    def refresh(self):
        ...
        loopc = tc.get("loop", {})
        self.var_escorts.set(str(loopc.get("max_escorts", 3)))
        self.var_time.set(str(loopc.get("time_limit_min", 60)))
        ...
        _ready, txt, color = calib_status(regions=regions, templates=templates, task_name=self.TASK_NAME,
                                          region_keys=need_r, tpl_keys=need_t, tpl_opt_keys=opt_tpl, label="运镖标定")
        self.lbl_calib.configure(text=txt, text_color=color)

    # ---- 参数保存（改动即保存）----
    def _on_max_escorts(self, raw):
        self._save_loop(raw, "max_escorts", lambda v: max(1, int(float(v))), "3", self.var_escorts)

    def _on_time_limit(self, raw):
        self._save_loop(raw, "time_limit_min", lambda v: max(0, int(float(v))), "60", self.var_time)
